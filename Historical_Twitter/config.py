
app_keys_tokens = [
    # those tokens are from example prject 
    {'consumer_key' : 'CP20F8yCMC85K26XY07w4XElp',
    'consumer_secret' : '4t1r4cdlBPGVzkosnZ2gvBqXbet5MbuJIlkuN0JKYufWIdo4yM',
    'access_token' : '1121041186305630208-hG4Jv9cfPOufx3vAgPpBUCODlWsHQH',
    'access_token_secret' : 'OJSXpMxZDzY9XUo2gqoqZcLUyGY1C9duopI4032fywDPb'}
    ,

    {'consumer_key' : '2BjmB9QN2UwT7BWGEYJc6mzyQ',
    'consumer_secret' : 'dkP4itLYIM0rqhHef4BiRkEgp8n2STc5CZuddYzjpnRzN3QX0m',
    'access_token' : '1121041186305630208-9pyRCJS3ltExpoKeTqKVrYcdSNnqHg',
    'access_token_secret' : 'dWIS8xzpbuB1T77UZSQCHJGBOX2uT7A82UmiwpyuSfrkq'}
    ,

    {'consumer_key' : 'W225IVMaLWc3Cio8Y2ZwHmwXT',
    'consumer_secret' : 'D0Gebz3e1xqrSKKCNbQPCwLsjNdQVZxHguLekTU4zCavWysswy',
    'access_token' : '1121041186305630208-vVcpClv576aYx9OJjVaWJkYA89m7eI',
    'access_token_secret' : 'ZjUk3ppAaudL4KR3oDQo3K6lDMZRKrnGvj2wYRpzfx1uP'}
    ,

    {'consumer_key' : 'ahKRXTnEizWqy4oHC4uBFxWuu',
    'consumer_secret' : 'xF2Pc3JwGtSij9Ig0UhW5A5o4RVk1kxcbTk6jMGM7W7XfOub8w',
    'access_token' : '1121041186305630208-85TVCtBvNc3RjW9RjmcBdwJn5FKxQm',
    'access_token_secret' : 'l3qRsugZsCt1MApDSjtCwMFS19Jms2Y2QiGpUPfzeWVit'}
    ,

    {'consumer_key' : 'IaRuxafuggm6eZvXdmurzA2MV',
    'consumer_secret' : '9DwkZsuQHFHtnFD5JVfttZ6uNkqsm99yNxvpmPMgSlPxAtqjJ9',
    'access_token' : '1121041186305630208-UJOUSKINRytWJY9OiCF9ANeqNacdMY',
    'access_token_secret' : 'kQjU0Vc4x7oPnSeuOM4Jrs5waE21dtYOKHN9Wi0hcikxB'}
    ,

    {'consumer_key' : 'W2pCfanNy4x8YiEEuC1pHbPU8',
    'consumer_secret' : '3XfspXNraGydG3xKYfxZ6wLVaJsCZqYeSx0cOUFSv2ABNHNi8a',
    'access_token' : '1121041186305630208-iHTAQCyxr0QcCPjEH8gpNYTZyomfjQ',
    'access_token_secret' : 'nsNp6LLbb1SacGUbKlUPBFBEiV937Lct9LxHIgB5MYKnL'}
    ,

    {'consumer_key' : 'kpC53YdFfY1Q4vXIEhLKM4lhT',
    'consumer_secret' : '3qYR9E9oucDhQSwilPQxUVnmdr1tJwAOi0iMyKCYWy63coEQLZ',
    'access_token' : '1121041186305630208-bFfc3Y4x0ueoHCyCvVUZxeMWRmhloR',
    'access_token_secret' : 'O460teRRz2MJxQRto7ipMz3MyKA2fwZWwW4bMnMVKzKGZ'}
    ,

    {'consumer_key' : 'hVmTeuwuAy44Ufr6q6SlIlSST',
    'consumer_secret' : 'BsYDcvgtmbhdGDv57oI5Q3DwSsBEuPtjaIDjALelQsmnQaeQf1',
    'access_token' : '1121041186305630208-9xcHlMmIVGemqa6jNPDYfrXOv05V8l',
    'access_token_secret' : 'NULMzSH41CSFTzJSv8rUu0fPf2VmiqqKX80SqF06LkqT0'}
    ,
    
    #those follow tokens from jingjing 
    {'consumer_key' : 'qFG8UE2HNdKLxrQqGmRuvM2GW',
    'consumer_secret':'UqSbasxKHPofBxIVj7FdE8iLMgqBkRnw5j0FmuZXdD3bwlNnRu',
    'access_token':'1354151035-18BfMJ0MpptHyMPAoXKJtct2Y79ADzxFh55KQQl',
    'access_token_secret':'X2AEvMCwfg0doGu8hdZ5T5GBVln8yuzgvvRD4VtuPfC8k'}
    ,
    
    {'consumer_key': 'VoOUcfsuU2Xw74Dp6OmdVaAx5',
     'consumer_secret':'t1OVXZM4VXcdDArFykv7uDx65GOutpD9QeV3VMKXplSpYUvRsq',
     'access_token':'1354151035-fUiaFz46uoToUQC3JwFJLTt7O7QshSHSqctAOEs',
     'access_token_secret':'dCPVUfsGp9rA8PRbHRLIEOUzV34WFR6r3S6U7ox1T2Zc2'}
    ,

    {'consumer_key':'rKju67MkpCg55cOmGiEmi5efO',
    'consumer_secret':'tqY3bYyRZHa6tJCCpehbroOifndV1rgmNU82qq0UID4ukQkynh',
    'access_token':'3009384632-HVYfglhGG3XNlBTLTdVnn5uj5EBBuoqqrGPlbgV',
    'access_token_secret':'27HLpkVdEEwaVk4A6yuzJDHxwcWpUMYX9gfq9bIE4YSlt'}
    ,
    
    {'consumer_key':'RXNJkCTQF4FXdyISue0M5RCSj',
    'consumer_secret':'kXSzoVBEU3bnFweWHE624NPn3T7GrGilwfbm1EBCzqR1CAYYCl',
    'access_token':'1354151035-rHMuSXWGSWDV6XPlOLIhtK3lKueMCXPzOXtse6G',
    'access_token_secret':'3AkzeTCPcVcqT5cDwndcjpN9nBhmHB5vMuQLZhjQYjfOd'}
    ,
    
    #those tokens from our group 
    {'consumer_key': 'xvgW0raEWPbnMZJ90FsRNa6oJ',
    'consumer_secret' : 'AfeEWoRjyjkasf7fMfJ9hTsoRdfVkYoF7YdUYgVzPN5n0KDT81',
    'access_token' :'1157192110572838913-kWvqynhqHzRb20vqFTUT96EJ4mf5Jh',
    'access_token_secret' : 'nKO4DaMsKrK91n0ncawIBaUVTeCN9a74EuvFrtooQL3ui'} 
    ,
    
    {'consumer_key':'Te4BDd3JhD1Iht7cHs7CENwR0', 
    'consumer_secret':'VUfUts4fknQMEQF5VDpcPhjH0jKD9F63FnRvEsSIhEPLMaunUj',
    'access_token':'1157192110572838913-HOpMxVvJfeEEiTbmc7XEeUHBPvIoDv',
    'access_token_secret':'b3DaC2by6nlHXpEfyISVjBPf3NnwICIIHoApV0mtXpmX1'}
    ,
    
    {'consumer_key': 'U2uNiJq0Bxoz184ZOfWekIRvt',
    'consumer_secret':'TkWGIy1Q3giZ71E48gSfgRpc554ddgZj7604c9WrmCWICwjlZZ',
    'access_token':'1158568035554222081-kVlFw0tuVZ6qN9BlP61R5b9hJ3Tyr0',
    'access_token_secret':'ctTKsH0RKdjySWaBpldjLQCuh6MtGGi6jWeL3kXcQDNdo'}
    ,
    
    {'consumer_key': 'NtrTatMZ27490GYOpdVv3S9dC',
    'consumer_secret':'cTVP7Tnqt3BhoEmlP1ST2tqjkWYzWDvxq4YATnKe4E8SN6U9Fr',
    'access_token':'1158568035554222081-gTJ4rgoeLYyLIE6ZUYyUhPGriKCJYz',
    'access_token_secret':'E083pnvgpnE2aVB87iX3e3k9OqduWIqmxww6PgrDZmkJN' }
    ,

    #for searchByName 16,17,18
    {'consumer_key': 'UlcKpGAMU5fW9uHi1xmEHlfF1',
     'consumer_secret': 'sHRmEho9FwnOjHYzaNFj010DR0YyoCdW7Ino1l13L9EfeWCr52',
     'access_token': '925304770193104897-XMkXssOdzi2Olfw9cBA4YhCYFq4Nl4P',
     'access_token_secret': 'NR1OQYnZeM756wbfgXClNG3VCXKSBu8rTfD8UMK7uaeHR'}
    ,

    {'consumer_key':'kiLz9KGKoly1YqlFniL91Avcl',
    'consumer_secret':'Ff7NxXzN9eUrHOWyWyjlscXwLSC3pUMwYBaVMmy37mOe6yNVUg',
    'access_token': '925304770193104897-UiszSjN3pO0faPZNyd1E6bwgEW68jJy',
    'access_token_secret' :'mqIOj8W9oYNSoqY5KrX1GmPGEXZkTq9IYSNsfsJVRS1k5'}
    ,

    {'consumer_key':"JKzUa3F0yiNLq460kORxe0KM2",
    'consumer_secret':"1efpSOFxA0VF3IIOor16jIguTcOOIdO9dd8znDQRvE6fRQY4FT",
    'access_token': "757469703547879424-PQCqCVPhItFqIljX7PxSH6BDbyqg5Mh",
    'access_token_secret': "s146GWiClnBv7s2VOBsEMzZjTnNTj82LX9H9kWT8k7LDY"}
    ]

geocodes =  {
            'sydney':"-33.63,150.89,85km",
            'melbourne':"-37.80,145.11,75km",
            'brisbane':"-27.33,152.81,109km",
            'perth':"-32.12,115.68,75km"
            }

coordinates = {
            #cities of AU
            'melbourne':[144.36,-38.50,145.88,-37.18],
            'brisbane':[152.10,-28.34,153.55,-26.45],
            'perth':[115.40,-32.80,116.41,-31.45],
            'adelaide': [138.4362, -35.3503, 138.8480, -34.5716],
            'sydney':[150.2815, -34.1202, 151.3430, -33.5781],
            'canberra':[148.6964,-35.6028,149.5631,-35.0358],

            #cities of melbourne 
            'calton':[144.956177,-37.80792,144.97608,-37.79226],
            'south_yarra':[144.975788,-37.8481,145.00644,-37.827676],
            'kew':[144.99927,-37.817374,145.06252,-37.78558],
            'bundoora':[145.0289,-37.72918,145.10364,-37.67647],
            'preston':[144.978979,-37.75571,145.03827,-37.72792],
            'cbd':[144.955012,-37.819944,144.974949,-37.80502],
            'clayton':[145.10752,-37.931418,145.151615,-37.89915],
            'doncaster':[145.099931,-37.800281,145.180155,-37.769982],
            'camberwell':[145.059686,-37.84945,145.100696,-37.826911],
            'brighton':[144.98176,-37.929127,145.030874,-37.897201]
            }

coordinates_str = {
            #cities of AU
            'melbourne':'144.36,-38.50,145.88,-37.18',
            'brisbane':'152.10,-28.34,153.55,-26.45',
            'adelaide': '138.4362, -35.3503, 138.8480, -34.5716',
            'sydney':'150.2815, -34.1202, 151.3430, -33.5781',

            #cities of melbourne
            'south_yarra':'144.975788,-37.8481,145.00644,-37.827676',
            'bundoora':'145.0289,-37.72918,145.10364,-37.67647',
            'cbd':'144.955012,-37.819944,144.974949,-37.80502',
            'clayton':'145.10752,-37.931418,145.151615,-37.89915'
            }

L_name=['melbourne','brisbane','perth','adelaide','sydney','canberra']

COUCHDB_URL = 'http://45.113.232.90/couchdbro/twitter/_design/twitter/_view/summary'
COUCHDB_USERNAME = 'readonly'
COUCHDB_PASSWORD = 'ween7ighai9gahR6'
COUCHDB_LIMIT = None



